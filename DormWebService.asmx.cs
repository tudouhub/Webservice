...
namespace Webservice
{
    ...
    public class DormWebService : System.Web.Services.WebService
    {
        ...
        {
            string sql = "select * from(select row_number()over(order by ID)as RowID,* from employee )as a where a.RowID between (@pageIndex-1)*@pageNumber+1 and @pageIndex*@pageNumber ";
            SqlParameter[] pms = new SqlParameter[]
            {
